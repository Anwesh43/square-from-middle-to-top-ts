...
class SFMTNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SFMTNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSFMTNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SFMTNode {
        var curr : SFMTNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SquareFromMiddleToTop {

    curr : SFMTNode = new SFMTNode(0)
    dir : number = 1 
